    ...
    def setUp(self):
        ...
        if self.tokenAuthMode:
            self.g = github.Github(
                self.oauth_token, per_page=self.per_page, retry=self.retry, pool_size=self.pool_size
            )
        elif self.jwtAuthMode:
            self.g = github.Github(
                jwt=self.jwt, per_page=self.per_page, retry=self.retry, pool_size=self.pool_size
            )
        else:
            self.g = github.Github(
                self.login, self.password, per_page=self.per_page, retry=self.retry, pool_size=self.pool_size
            )
